...
class BlueprintContent extends VBox {
    ...
    BlueprintContent(final Blueprint blueprint) {
    ...
    private void initComponents() {
        ...
        initIngredients();
        if (this.blueprint instanceof EngineerBlueprint engineerRecipe && engineerRecipe.getBlueprintName().equals(BlueprintName.ENGINEER_D3)) {
            initSteps();
        }
    ...
    private Wishlists loadCommanderWishlists(final Commander commander) {
        ...
        final List<MenuItem> menuItems = wishlists.getAllWishlists().stream().sorted(Comparator.comparing(Wishlist::getName)).map(wishlist -> {
            ...
            menuItem.setOnAction(event -> EventService.publish(new WishlistBlueprintEvent(commander.getFid(), wishlist.getUuid(), List.of(new WishlistBlueprint(this.blueprint.getBlueprintName(), true)), Action.ADDED)));
    ...
    private void initEngineers() {
        ...
        final Label[] engineerLabels = ((ModuleBlueprint) this.blueprint).getEngineers().stream()
    ...
    private List<MaterialIngredient> getRecipeIngredients(final Blueprint blueprint, final Class<? extends OdysseyMaterial> materialClass, final OdysseyStorageType storageType, final Map<? extends OdysseyMaterial, Storage> materialMap) {
