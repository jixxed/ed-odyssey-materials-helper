...
public class HorizonsCommoditiesOverview extends DestroyableVBox implements DestroyableEventTemplate {
    ...
    private void update() {
        this.getNodes().clear();
        final List<DestroyableHBox> rows = Arrays.stream(CommodityType.values()).filter(Predicate.not(CommodityType::isUnknown)).map(commodityType -> {
    ...
    private DestroyableHBox createCommodityCardRow(final CommodityType type, final HorizonsCommodityCard[] array) {
        ...
        final DestroyableFlowPane commodities = FlowPaneBuilder.builder()
                .withNodes(array)
                .build();
        commodities.addBinding(commodities.vgapProperty(), ScalingHelper.getPixelDoubleBindingFromEm(0.25));
        commodities.addBinding(commodities.hgapProperty(), ScalingHelper.getPixelDoubleBindingFromEm(0.25));
