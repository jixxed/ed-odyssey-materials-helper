...
public class ModulesLayer extends Pane implements Template {
...
//        AnchorPane.setLeftAnchor(this.scrollPane, 0D);
    ...
    public void initShipSlots() {
        ...
        if (APPLICATION_STATE.getShip() != null) {
            this.hardpointsVbox.getChildren().add(LabelBuilder.builder().withNonLocalizedText("hardpoints").build());
            this.coreVbox.getChildren().add(LabelBuilder.builder().withNonLocalizedText("core").build());
            this.optionalVBox.getChildren().add(LabelBuilder.builder().withNonLocalizedText("optional").build());
            this.utilityVbox.getChildren().add(LabelBuilder.builder().withNonLocalizedText("utility").build());
    ...
    public void showStatsPopover(SlotBox slotBox) {
        if (statsPopover != null) {
            statsPopover.hide();
        }
        statsPopover = new PopOver();
        statsPopover.setContentNode(new ModuleDetails(slotBox.getSlot()));
        statsPopover.show(slotBox);
    }
