...
public class CSVResourceBundle extends ResourceBundle {
    ...
    private CSVResourceBundle(final String baseName, final Locale locale) {
        try (final CSVParser csvParser = CSVFormat.DEFAULT.parse(new InputStreamReader(Objects.requireNonNull(CSVResourceBundle.class.getResourceAsStream("/" + baseName + ".csv")), StandardCharsets.UTF_8))) {
