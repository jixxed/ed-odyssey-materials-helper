...
//        OrderService.clearOrders();
        ...
        SELL_ORDER_PARSER.parseOdyssey(data.getSquadronCarrier().getOrders().getOnfootmicroresources().getSales(), StoragePool.SQUADRONCARRIER);
        BUY_ORDER_PARSER.parseOdyssey(data.getSquadronCarrier().getOrders().getOnfootmicroresources().getPurchases(), StoragePool.SQUADRONCARRIER);
...
//        BUY_ORDER_PARSER.parse(data.getSquadronCarrier().getOrders().getCommodities().getPurchases());
        ...
        ASSET_PARSER.parse(data.getSquadronCarrier().getCarrierLocker().getAssets(), StoragePool.SQUADRONCARRIER);
        GOOD_PARSER.parse(data.getSquadronCarrier().getCarrierLocker().getGoods(), StoragePool.SQUADRONCARRIER);
        DATA_PARSER.parse(data.getSquadronCarrier().getCarrierLocker().getData(), StoragePool.SQUADRONCARRIER);

        COMMODITY_PARSER.parse(data.getSquadronCarrier().getCargo(), StoragePool.SQUADRONCARRIER);
