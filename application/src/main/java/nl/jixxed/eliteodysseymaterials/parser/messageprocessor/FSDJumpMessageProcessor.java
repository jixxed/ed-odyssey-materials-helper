...
public class FSDJumpMessageProcessor implements MessageProcessor<FSDJump> {
    ...
    public void process(final FSDJump event) {
        ...
        if (!starSystem.isBlank()) {
            ...
            EventService.publish(new FSDJumpJournalEvent(event, new StarSystem(starSystem, SystemEconomy.forKey(economy), SystemEconomy.forKey(secondEconomy), SystemGovernment.forKey(government), SystemSecurity.forKey(security), SystemAllegiance.forKey(allegiance), factionState, x, y, z), body));
        }
