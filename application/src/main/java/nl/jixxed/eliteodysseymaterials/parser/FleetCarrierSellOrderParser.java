...
public class FleetCarrierSellOrderParser {
    ...
    public void parseOdyssey(Map<String, SaleItem> sales) {
        if(sales == null) {
            return;
        }
        sales.forEach((s, saleItem) -> {
            try {
                OrderService.addSellOrder(OdysseyMaterial.subtypeForName(saleItem.getName()), saleItem.getPrice().intValue(), saleItem.getStock().intValue());
    ...
    public void parse(List<Sale> sales) {
        if(sales == null) {
            return;
        }
        sales.forEach(sale -> {
            try {
                OrderService.addSellOrder(HorizonsMaterial.subtypeForName(sale.getName()), Integer.parseInt(sale.getPrice()), Integer.parseInt(sale.getStock()));
